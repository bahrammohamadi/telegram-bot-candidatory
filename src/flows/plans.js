...
const PLAN_MARKETING = {
  free: {
    ...
    benefits: [
      "✅ ارزیابی آمادگی کامل ۹ مرحله‌ای",
    ...
    limitations: [
      "🔒 برنامه و اقدامات فوریِ قابل‌اجرا",
      "🔒 تحلیل SWOT و داشبورد کمپین",
      "🔒 تولید محتوا (پست، شعار، پیامک)",
      "🔒 تب‌های کامل آموزشی (نکات، تمرین، اشتباهات)",
    ],
    cta: "🎯 شروع رایگان",
    closingPitch: "_امتیاز کلی‌تان را رایگان ببینید؛ برای «برنامه‌ی دقیق بهبود» ارتقا دهید._",
  },
  starter: {
    badge: "🚀",
...
// قیمت‌ها — منبع حقیقت واحد: constants/plans.js
// (به‌جای تعریف مجدد و ناهماهنگ، از همان‌جا ساخته می‌شوند تا قیمت‌ها در
//  کل پروژه یکسان بمانند.)
const PLAN_PRICES = Object.fromEntries(
  ["free", "starter", "professional", "vip"].map((id) => {
    const p = PLANS[id] || PLANS.free;
    return [id, { display: p.priceLabel || "—", numeric: p.price || 0 }];
  })
);
