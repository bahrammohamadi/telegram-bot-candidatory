...
// ─── هندلرهای ۹ مرحله مشاوره هوشمند — CommonJS ───
// نسخه اصلاح‌شده: ۱۴۰۴/۱۲/۰۸ — استفاده از lastInteractionNew + پیام خطای اختصاصی + بهبود UX
...
// ثابت‌های وضعیت
const ST_SUMMARY = 99;      // خلاصه پاسخ‌ها
const ST_DONE = 200;        // تکمیل‌شده
// ویرایش: 100 + stepIndex (مثلاً 103 = ویرایش مرحله ۳)
...
/**
 * نمایش یک مرحله (با پشتیبانی از callback و پیام معمولی)
 */
async function showStep(ctx, userId, idx) {
  ...
  try {
    ...
    console.error("خطا در نمایش مرحله:", e.message);
    await ctx.reply("⚠️ خطایی رخ داد. لطفاً دوباره امتحان کنید.", { reply_markup: mainMenuKB() });
  }
}

/**
 * شروع مشاوره (فاز اصلی)
 */
async function handleStartConsultation(ctx) {
  ...
  // اگر کد ملی و تلفن قبلاً ثبت شده → رد شدن
  let startIdx = 0;
...
/**
 * ورودی متنی (هندل پیام‌های معمولی)
 */
async function handleTextInput(ctx) {
  const userId = String(ctx.from.id);
  const user = await getOrCreateUser(userId, ctx.from);

  // اگر در حالت خلاصه یا تمام‌شده هستیم → نادیده بگیر
  if (
...
async function showSummary(ctx, userId, answers) {
  ...
  try {
    ...
    console.error("خطا در نمایش خلاصه:", e.message);
  }
}
...
async function handleConfirm(ctx) {
  ...
  // چک کردن تکمیل همه مراحل اجباری
  const missing = [];
  for (let i = 0; i < TOTAL_STEPS; i++) {
    const s = STEPS[i];
  ...
  try {
    ...
    console.error("خطا در ارسال گزارش:", e.message);
...
async function handleCancelConsultation(ctx) {
  ...
  try {
    ...
    console.error("خطا در لغو:", e.message);
  }
}
