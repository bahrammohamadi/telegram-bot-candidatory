...
bot.command("start", async (ctx) => {
  await ctx.reply("Welcome to Candidatory Bot!\nPlease use the menu below:", { reply_markup: mainMenuKB() });
});

bot.command("menu", async (ctx) => {
  await ctx.reply("Main Menu:", { reply_markup: mainMenuKB() });
});
...
bot.callbackQuery("menu", async (ctx) => {
  try {
    await ctx.editMessageText("Main Menu:", { reply_markup: mainMenuKB() });
  } catch {
    await ctx.reply("Main Menu:", { reply_markup: mainMenuKB() });
  }
...
bot.on("message:text", async (ctx) => {
  ...
  await ctx.reply("Please use the menu or type /start", { reply_markup: mainMenuKB() });
});
...
# ۵. اگه OK بود push کن
git add src/main.js
git commit -m "fix: replace main.js via terminal"
